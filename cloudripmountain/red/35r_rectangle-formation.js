...
for (var x = soldierX; x <= soldierX + width; x += step) {
    for (var y = soldierY; y <= soldierY + height; y += step) {
        hero.summon("soldier");
...
for (var x = archerX1; x <= archerX2; x += step) {
    for (var y = archerY1; y <= archerY2; y += step) {
        hero.summon("archer");
        var lastUnit = hero.built[hero.built.length-1];
        // Command the last built unit by using the  x/y variables:
        hero.command(lastUnit, "move", {x: x, y: y})
